...
HRESULT CSegment::GetSegment(IDirectMusicSegment **ppSegment)
{
	...
	//CHANGE: Jason Hendrickson: Why get the segment from the state? This only works when the segment is running
	//        Checked interface pointer returned - it's equivalent to m_pSegment
	//OLD:
	...
	//NEW:
	DM_ASSERT(strMembrFunc,__LINE__,m_pSegment==NULL,ppSegment==NULL);
